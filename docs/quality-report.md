...
`activate_users.js` en la raíz del repo (activa a todos los usuarios en bloque) no está en `package.json`, no tiene ningún comentario explicando cuándo usarlo, y no valida ningún permiso (es un script de línea de comandos con acceso directo a la DB, no un endpoint). Bajo riesgo tal como está (requiere acceso al servidor/repo para ejecutarlo), pero debería vivir en una carpeta `scripts/` documentada o eliminarse si ya no se usa.
